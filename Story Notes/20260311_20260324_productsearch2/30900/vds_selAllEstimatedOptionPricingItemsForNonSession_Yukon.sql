...
GO
ALTER procedure [dbo].[vds_selAllEstimatedOptionPricingItemsForNonSession_Yukon]
...
/*
		...
		Nov 2015 - commented out the COLLATE keyword to remove cap-sensitive comparison on plan name
...
FROM
    ...
    LEFT JOIN spec_communities sc WITH (NOLOCK)
        ON sc.spec_id = sm.spec_id
    LEFT JOIN spec_series ss WITH (NOLOCK)
        ON ss.spec_id = sm.spec_id
    JOIN plan_mstr pm WITH (NOLOCK)
        ON pm.spec_id = sm.spec_id
    JOIN pricesets p WITH (NOLOCK)
        ON p.spec_id = sm.spec_id
WHERE
...
------------------------------------------------------------
-- Determine the correct (max) effective date
------------------------------------------------------------
...
from 
	...
	LEFT JOIN @MaxMinBuilds mmb on mmb.max_build_id = pl.build_id
where 
	pl.plan_id = @plan_id
	and pm.spec_id = @active_spec_id
	and pm.plan_name = @unmapped_plan_name --COLLATE SQL_Latin1_General_CP1_CS_AS
	and pl.effective_date = @prices_landed_effective_date 
	and pl.build_id in (select max_build_id from @MaxMinBuilds)
	and pbm.bill_qty > 0
